...
Say there has been a SIEM alert. A security analyst has to use a playbook to respond. The first action is to _assess the alert_, i.e., to determine if the alert is actually valid by identifying why it was generated by the tool. This can be done by analyzing log data and related metrics. Next, we _use the actions and tools specified in the paybook to contain the malware and reduce further damage_.
